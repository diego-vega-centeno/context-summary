...
export default function PRCard({ pr, refreshingPR, refreshPR }: PRCardProps) {
  ...
  return (
    <Link href={`stories/${pr.id}`} className="h-full">
      <div
        key={pr.id}
        className="flex flex-col justify-between text-sm border-1 border-border rounded-lg p-4 pb-2 hover:bg-hover hover:cursor-default h-full"
      >
        <div className="">
          ...
          <div className="flex-1">
            ...
            <div className="text-foreground py-2">{pr.current_state}</div>
          </div>
        </div>
        <div>
