...
The input data is an array of timeline of events, each one with this type of structure:

INPUT DATA:
	1. METADATA: {title, description, author, state, created_at}
...
- what_was_built: Use the metadata.description to explain the 'summary' (what) and 'context' (why/the problem it solves).
- key_decisions: Look for 'reviewed' or 'commented' events where technical directions were chosen. Cite the user and the event.
- blocking_points: Identify 'changes_requested' review states or comments mentioning blockers. State exactly who/what is being waited on.
- current_state: A summary of the very last few events in the timeline.
...
[
    ...
    {
      type: "merged" | "closed" | "reopened" | "ready_for_review",
      user: entry.actor.login,
      timestamp: entry.created_at,
    },
    {
